...
def score_ev_ebitda(ev_ebitda):
    ...
    if ev_ebitda <= 10: return 5
    elif ev_ebitda <= 12: return 4
    elif ev_ebitda <= 14: return 3
    elif ev_ebitda <= 18: return 2
    else: return 1

def score_roe(roe):
    """Scores Return on Equity (ROE): Higher is better."""
    if roe >= 20: return 5
    elif roe >= 15: return 4
    elif roe >= 10: return 3
    elif roe >= 5: return 2
    else: return 1

def score_rsi(rsi):
    """Scores RSI (Relative Strength Index): Detects overbought/oversold conditions."""
    if rsi < 40: return 5  # Oversold (buy signal)
...
def score_volatility(volatility):
    ...
    elif volatility <= 3: return 4
    elif volatility <= 4: return 3
    elif volatility <= 5: return 2
    else: return 1
